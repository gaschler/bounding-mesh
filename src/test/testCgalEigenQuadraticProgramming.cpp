...
class InternalTimer {
 public:
  InternalTimer() { this->elapsedTime = MyClock::duration::zero(); }

  MyClock::duration getElapsedTime() { return this->elapsedTime; }
  ...
  void start() { this->startTime = MyClock::now(); }
...
class ProfilerScopedTimer {
 public:
  ProfilerScopedTimer(const std::string& name) : name(name) {
    this->timer.start();
  }

  ~ProfilerScopedTimer() {
    this->timer.stop();
    std::cout << "Time: " << this->name << " took "
              << this->timer.getElapsedSeconds() * 1000 << " ms." << std::endl;
  }
...
int main(int argc, char** argv) {
  ...
  for (int s = 0; s < 5; ++s) {
    ...
    std::cout << "Solving " << numRepetitions << " repetitions." << std::endl;
    {
      ...
      for (int rep = 0; rep < numRepetitions; ++rep) {
        ...
        if (rep == 0) {
          ...
          std::cout << "y: " << CGAL::to_double(s.objective_value())
                    << std::endl;
        }
      }
    }

    {
      ...
      for (int rep = 0; rep < numRepetitions; ++rep) {
        typedef CGAL::Quadratic_program_from_iterators<
            Real**, Real*, CGAL::Const_oneset_iterator<CGAL::Comparison_result>,
            bool*, Real*, bool*, Real*, Real**, Real*>
            ProgramIt;
        int numVariables = 3;
        int numConstraints = constraints.size();
        std::vector<Real> Ax(numConstraints), Ay(numConstraints),
            Az(numConstraints);
        ...
        Real c[] = {qem(3, 0), qem(3, 1),
                    qem(3, 2)};  // FIXME: does not solve the right values
        Real c0 = qem(3, 3);
        bool fl[] = {false, false};
        Real l[] = {0, 0};
        bool fu[] = {false, false};
        Real u[] = {0, 0};
        ProgramIt qp(numVariables, numConstraints, A, &b[0], r, fl, l, fu, u, D,
                     c, c0);
        CGAL::Quadratic_program_solution<Real> s =
...

    {
      ...
      Real cost = (x.transpose() * G * x)(0, 0) +
                  (2 * g0.transpose() * x)(0, 0) + qem(3, 3);
